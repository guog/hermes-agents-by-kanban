...
def render_card_body(card: CardRecord) -> str:
    ...
    return (
        ...
        "非 TEST 卡不得设置 `test_disposition`/`skip_reason`；lint、build 和"
        "文档检查写入 `verification`。完成前必须把 metadata 保存为 JSON，执行 "
        "`hollysysctl validate-completion --card-id <当前卡ID> "
        "--metadata <JSON文件>`，确认 `ok=true` 后把同一对象原样传给 "
        "`kanban_complete`。\n\n"
