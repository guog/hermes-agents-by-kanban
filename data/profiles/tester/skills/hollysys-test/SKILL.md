...
1. 调用 `kanban_show()`；要求 `created_by=hollysys-controller`，并验证卡片 JSON 的 v3 protocol、mode、run/stage/iteration/assignee/parent、项目、worktree、分支、MR 和全部冻结基线。开始测试前读取 GitLab 当前 MR head。
...
7. 使用严格 v7 metadata 完成，必含完整上下文；pass/fail 都必须将 `head_sha`、
