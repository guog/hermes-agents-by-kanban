...
新增或移除禁用项必须由人类审查后同步修改全部 Profile 的 `skills.disabled`，并在 `scripts/verify-bundle.sh` 的统一禁用集断言中更新。不要直接修改 `/opt/hermes/skills`，也不要在单个 Profile 的 Dashboard 中形成未回写仓库的长期漂移。
...
Dashboard 的 Profile 切换器也可以查看和临时切换 Skill；变更在新会话或 Gateway 重启后生效。仓库配置仍是部署权威来源，接受这次托管配置时按升级流程使用一次 `FLEET_FORCE_CONFIG=1`，确认后恢复为 `0`。
...
6. `config.yaml`：只启用必要工具，并完整复制 1.3.1 节统一的 `skills.disabled`，同时保留 fleet 的模型、`terminal.cwd: /workspace/projects`、`terminal.home_mode: profile`、`worktree: false`、中文显示和写入审批策略。独立 Gateway 设置 `kanban.dispatch_in_gateway: false`，避免它自行推进正式交付卡片。
